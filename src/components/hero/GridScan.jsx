...
// face-api.js is ~146KB gzipped and drags in @tensorflow/tfjs-core. As a static
// import it sat in this component's chunk, so the grid could not paint its first
// frame until the whole face-detection library had downloaded and parsed.
// Loaded on demand instead: only when enableWebcam is set, and only once the
// main thread is idle.
...
export const GridScan = ({
  ...
  // Fired once, after the first frame is actually on screen. Held in a ref so
  // passing a fresh callback each render can't retrigger the scene-setup effect.
  const onReadyRef = useRef(onReady);
  onReadyRef.current = onReady;
  const readyFiredRef = useRef(false);

  // Holds the lazily-imported face-api module once it lands.
  ...
  useEffect(() => {
    // Nothing to fetch when face tracking is off — this used to download the
    // library and both model files regardless of enableWebcam.
    ...
    // Defer to an idle moment so parsing the library never competes with the
    // grid's first frames. The timeout caps how long we wait for idle.
    const schedule = window.requestIdleCallback
      ? window.requestIdleCallback.bind(window)
      : cb => setTimeout(cb, 700);
    const unschedule = window.cancelIdleCallback
      ? window.cancelIdleCallback.bind(window)
      : clearTimeout;
    const handle = schedule(load, { timeout: 2500 });
